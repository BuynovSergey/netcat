...
final class SvgImageBackEnd implements ImageBackEndInterface
{
    ...
    protected function round($n, $precision) {
        return str_replace(',', '.', round($n, $precision));
    }
    ...
    public function scale(float $size) : void
    {
        ...
        $this->xmlWriter->writeAttribute(
            'transform',
            sprintf('scale(%s)', $this->round($size, self::PRECISION))
        );
    ...
    public function translate(float $x, float $y) : void
    {
        ...
        $this->xmlWriter->writeAttribute(
            'transform',
            sprintf('translate(%s,%s)', $this->round($x, self::PRECISION), $this->round($y, self::PRECISION))
        );
        ++$this->stack[$this->currentStack];
    }
        ...
        foreach ($path as $op) {
            switch (true) {
                case $op instanceof Move:
                    $pathData[] = sprintf(
                        'M%s %s',
                        $this->round($op->getX(), self::PRECISION),
                        $this->round($op->getY(), self::PRECISION)
                    );
                    break;
                ...
                case $op instanceof EllipticArc:
                    $pathData[] = sprintf(
                        'A%s %s %s %u %u %s %s',
                        $this->round($op->getXRadius(), self::PRECISION),
                        $this->round($op->getYRadius(), self::PRECISION),
                        $this->round($op->getXAxisAngle(), self::PRECISION),
                        $op->isLargeArc(),
                        $op->isSweep(),
                        $this->round($op->getX(), self::PRECISION),
                        $this->round($op->getY(), self::PRECISION)
                ...
                case $op instanceof Curve:
                    $pathData[] = sprintf(
                        'C%s %s %s %s %s %s',
                        $this->round($op->getX1(), self::PRECISION),
                        $this->round($op->getY1(), self::PRECISION),
                        $this->round($op->getX2(), self::PRECISION),
                        $this->round($op->getY2(), self::PRECISION),
                        $this->round($op->getX3(), self::PRECISION),
                        $this->round($op->getY3(), self::PRECISION)
                    );
                    break;
    ...
    private function createGradientFill(Gradient $gradient, float $x, float $y, float $width, float $height) : string
    {
        ...
        switch ($gradient->getType()) {
            case GradientType::HORIZONTAL():
                $this->xmlWriter->writeAttribute('x1', (string) $this->round($x, self::PRECISION));
                $this->xmlWriter->writeAttribute('y1', (string) $this->round($y, self::PRECISION));
                $this->xmlWriter->writeAttribute('x2', (string) $this->round($x + $width, self::PRECISION));
                $this->xmlWriter->writeAttribute('y2', (string) $this->round($y, self::PRECISION));
                break;

            case GradientType::VERTICAL():
                $this->xmlWriter->writeAttribute('x1', (string) $this->round($x, self::PRECISION));
                $this->xmlWriter->writeAttribute('y1', (string) $this->round($y, self::PRECISION));
                $this->xmlWriter->writeAttribute('x2', (string) $this->round($x, self::PRECISION));
                $this->xmlWriter->writeAttribute('y2', (string) $this->round($y + $height, self::PRECISION));
                break;
            ...
            case GradientType::INVERSE_DIAGONAL():
                $this->xmlWriter->writeAttribute('x1', (string) $this->round($x, self::PRECISION));
                $this->xmlWriter->writeAttribute('y1', (string) $this->round($y + $height, self::PRECISION));
                $this->xmlWriter->writeAttribute('x2', (string) $this->round($x + $width, self::PRECISION));
                $this->xmlWriter->writeAttribute('y2', (string) $this->round($y, self::PRECISION));
                break;

            case GradientType::RADIAL():
                $this->xmlWriter->writeAttribute('cx', (string) $this->round(($x + $width) / 2, self::PRECISION));
                $this->xmlWriter->writeAttribute('cy', (string) $this->round(($y + $height) / 2, self::PRECISION));
                $this->xmlWriter->writeAttribute('r', (string) $this->round(max($width, $height) / 2, self::PRECISION));
                break;
        }
